...
export default function DashboardPage({
  ...
  const kpiList: Kpi[] = [
    ...
    { title: "Total Promo Revenue", value: formatCurrency(kpis.totalPromoRevenue), icon: TrendingUp, description: "Gross revenue from all units sold during a promotion, before deducting discounts or costs." },
    { title: "Incremental Revenue", value: formatCurrency(kpis.incrementalPromoRevenue), icon: Plus, description: "The portion of revenue directly attributable to the promotion, above what would have been sold anyway (baseline)." },
    { title: "Incremental Sales Lift", value: formatPercentage(kpis.incrementalPromoSalesLift), icon: Percent, description: "Percentage increase in sales due to promotions, compared to the baseline." },
    { title: "Baseline Sales", value: formatCurrency(kpis.totalBaselineSales), icon: ShoppingCart, description: "Sales volume expected if no promotional activity had occurred, based on historical data and trends." },
    { title: "Promo Elasticity", value: formatNumber(kpis.promoElasticity), icon: GitMerge, description: "Sensitivity of sales volume to a 1% change in discount." },
    { title: "Average Discount %", value: formatPercentage(kpis.averageDiscount), icon: Percent, description: "Average percentage discount offered across all promotional sales." },
    { title: "Offer Effectiveness", value: formatNumber(kpis.offerEffectivenessIndex, 0), icon: Gift, description: "Index of overall offer performance, combining ROI and sales lift." },
    { title: "Promo ROI", value: formatPercentage(kpis.promoRoi), icon: Target, description: "Return on investment, calculated as (Incremental Revenue / Promo Spend)." },
